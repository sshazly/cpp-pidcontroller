...
class Gnuplot
{
    ...
    Gnuplot& plotfile_xy_multi(const std::string &filename,
                         int numPlots,
                         const std::string &title = "");
    ...
    ///   from data
    Gnuplot& plot_xy_multi(std::vector<std::vector<double>> XA, std::vector<std::vector<double>> YA, const std::string &title = "");
...
/// Plots a 2d graph from a list of doubles: x y
//
Gnuplot& Gnuplot::plot_xy_multi(std::vector<std::vector<double>> XA, std::vector<std::vector<double>> YA, const std::string &title)
{

	int numPlotsX = XA.size();
	int numPlotsY = YA.size();
    if (numPlotsX!=numPlotsY)
    {
        throw GnuplotException("input array sizes do not match up");
        return *this;
    };
	for (int i=0;i<numPlotsX;i++){
		std::vector<double> x=XA[i];
		std::vector<double> y=YA[i];
		int xS =x.size();
		int yS =y.size();
		if (xS == 0 || yS == 0)
		{
			throw GnuplotException("std::vector number too small");
			return *this;
		};

		if (xS != yS)
		{
			throw GnuplotException("Length of the std::vector number differs");
			return *this;
		};
	};
	std::vector<double> x=XA[0];
	std::vector<double> y=YA[0];
	int xS =x.size();
	int yS =y.size();
    std::ofstream tmp;
    std::string name = create_tmpfile(tmp);
    ...
    //
    for (int i = 0; i < xS; i++){
    	for (int j=0;j<numPlotsX;j++){
    		std::vector<double> x=XA[j];
    		std::vector<double> y=YA[j];
    		tmp << x[i] << " " << y[i] << " ";
    	};
    	tmp << std::endl;
    };
    ...
    plotfile_xy_multi(name, numPlotsX, title);
...
//
Gnuplot& Gnuplot::plotfile_xy_multi(const std::string &filename,
                              int numPlots,
                              const std::string &title)
{
    //
